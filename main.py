...
def ask_ans_User():
    ...
    if form.validate_on_submit():
        ...
        # Note, email in db is unique so will only have one result.
        user_quest = result_quest.scalar()
        result_mail = db.session.execute( db.select(User.email).where(User.email == form.email.data))

        # Note, email in db is unique so will only have one result.
        user_mail = result_mail.scalar()
        if not user_quest or not check_password_hash(user_quest.answer, answer):
            Mail.connection.sendmail(from_addr=Mail.my_email, to_addrs=user_mail,
                                     msg="Someone who trying to access your blog site account...!!!")

            if not user_quest:
                flash(f"Please Enter  Your  Correct Subjective Question.")
